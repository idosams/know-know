...
```
packages/
  core/       — @codegraph/core (parsers, indexer, query engine, types)
  cli/        — @codegraph/cli (Commander.js CLI)
  mcp-server/ — @codegraph/mcp-server (MCP protocol server)
```
...
CodeGraph indexes itself. After modifying @codegraph annotations:
...
- **@codegraph annotations** — JSDoc blocks with `@codegraph` YAML marker before imports
